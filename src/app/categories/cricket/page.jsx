...
export default async function Cricket() {
  ...
  // Filter only cricket arenas
  const cricketArenas = arenas.filter(arena => {
    const categories = arena.arenaDetails?.categories || []
    return categories.some(cat => cat.toLowerCase() === 'cricket')
  })

  return <CricketPage arenas={cricketArenas} />
}
